...
package company.android.documentmanager.office.fc.hslf.usermodel;
...
import company.android.documentmanager.office.fc.hslf.exceptions.CorruptPowerPointFileException;
import company.android.documentmanager.office.fc.hslf.model.HeadersFooters;
import company.android.documentmanager.office.fc.hslf.model.Hyperlink;
import company.android.documentmanager.office.fc.hslf.model.MovieShape;
...
import company.android.documentmanager.office.fc.hslf.record.Document;
import company.android.documentmanager.office.fc.hslf.record.DocumentAtom;
import company.android.documentmanager.office.fc.hslf.record.ExAviMovie;
import company.android.documentmanager.office.fc.hslf.record.ExControl;
import company.android.documentmanager.office.fc.hslf.record.ExHyperlink;
import company.android.documentmanager.office.fc.hslf.record.ExHyperlinkAtom;
import company.android.documentmanager.office.fc.hslf.record.ExMCIMovie;
import company.android.documentmanager.office.fc.hslf.record.ExObjList;
import company.android.documentmanager.office.fc.hslf.record.ExObjListAtom;
import company.android.documentmanager.office.fc.hslf.record.ExOleObjAtom;
import company.android.documentmanager.office.fc.hslf.record.ExVideoContainer;
import company.android.documentmanager.office.fc.hslf.record.ExtendedParagraphHeaderAtom;
import company.android.documentmanager.office.fc.hslf.record.ExtendedPresRuleContainer;
import company.android.documentmanager.office.fc.hslf.record.FontCollection;
import company.android.documentmanager.office.fc.hslf.record.HeadersFootersContainer;
import company.android.documentmanager.office.fc.hslf.record.PersistPtrHolder;
import company.android.documentmanager.office.fc.hslf.record.PositionDependentRecord;
...
import company.android.documentmanager.office.fc.hslf.record.RecordContainer;
import company.android.documentmanager.office.fc.hslf.record.RecordTypes;
import company.android.documentmanager.office.fc.hslf.record.SlideListWithText;
import company.android.documentmanager.office.fc.hslf.record.SlidePersistAtom;
import company.android.documentmanager.office.fc.hslf.record.ExtendedPresRuleContainer.ExtendedParaAtomsSet;
import company.android.documentmanager.office.fc.hslf.record.SlideListWithText.SlideAtomsSet;
    ...
    private void buildSlidesAndNotes()
    {
        ...
        if (masterSLWT != null)
        {
            ...
            for (int i = 0; i < masterSets.length; i++)
            {
                ...
                if (r instanceof company.android.documentmanager.office.fc.hslf.record.Slide)
                {
                    TitleMaster master = new TitleMaster((company.android.documentmanager.office.fc.hslf.record.Slide)r, sheetNo);
                    master.setSlideShow(this);
                    tmr.add(master);
                }
                else if (r instanceof company.android.documentmanager.office.fc.hslf.record.MainMaster)
                {
                    SlideMaster master = new SlideMaster((company.android.documentmanager.office.fc.hslf.record.MainMaster)r, sheetNo);
                    master.setSlideShow(this);
                    mmr.add(master);
                }
            }
        ...
        // notesSLWT
        company.android.documentmanager.office.fc.hslf.record.Notes[] notesRecords;
        SlideAtomsSet[] notesSets = new SlideAtomsSet[0];
        Hashtable<Integer, Integer> slideIdToNotes = new Hashtable<Integer, Integer>();
        if (notesSLWT == null)
        {
            // None
            notesRecords = new company.android.documentmanager.office.fc.hslf.record.Notes[0];
        }
        else
        {
            // Match up the records and the SlideAtomSets
            notesSets = notesSLWT.getSlideAtomsSets();
            ArrayList<company.android.documentmanager.office.fc.hslf.record.Notes> notesRecordsL = new ArrayList<company.android.documentmanager.office.fc.hslf.record.Notes>();
            for (int i = 0; i < notesSets.length; i++)
            {
                ...
                if (r instanceof company.android.documentmanager.office.fc.hslf.record.Notes)
                {
                    company.android.documentmanager.office.fc.hslf.record.Notes notesRecord = (company.android.documentmanager.office.fc.hslf.record.Notes)r;
            ...
            notesRecords = new company.android.documentmanager.office.fc.hslf.record.Notes[notesRecordsL.size()];
            notesRecords = notesRecordsL.toArray(notesRecords);
        }

        // Now, do the same thing for our slides
        company.android.documentmanager.office.fc.hslf.record.Slide[] slidesRecords;
        SlideAtomsSet[] slidesSets = new SlideAtomsSet[0];
        if (slidesSLWT == null)
        {
            // None
            slidesRecords = new company.android.documentmanager.office.fc.hslf.record.Slide[0];
        }
        else
        {
            // Match up the records and the SlideAtomSets
            slidesSets = slidesSLWT.getSlideAtomsSets();
            slidesRecords = new company.android.documentmanager.office.fc.hslf.record.Slide[slidesSets.length];
            for (int i = 0; i < slidesSets.length; i++)
            {
                ...
                if (r instanceof company.android.documentmanager.office.fc.hslf.record.Slide)
                {
                    slidesRecords[i] = (company.android.documentmanager.office.fc.hslf.record.Slide)r;
                }
                /*else
                {
